...
{
    public GameObject redBaloon, blueBalloon, greenBalloon, cube;
	// Use this for initialization
    private int redSize, blueSize, greenSize;
	void Start ()
	{
	    redBaloon = GameObject.Find("RedBalloonSphere");
        blueBalloon = GameObject.Find("BlueBalloonSphere");
        greenBalloon = GameObject.Find("GreenBalloonSphere");
        cube = GameObject.Find("ChangingCube");
	    redSize = blueSize = greenSize = (int) ((redBaloon.transform.localScale.x - 0.5) * 255);
	...
	void Update () {
	    if (Input.GetKey("q") && redSize >= 0)
	    {
	        ...
	        redBaloon.transform.localScale = new Vector3(balloonSize, balloonSize, balloonSize);
	    }
